...
<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-between mb-3">
            <button type="button" onclick="window.location.href='/admin/laporan/{{ $dataquiz->slug }}'"
                class="btn btn-primary btn-sm"><i class='bx bx-left-arrow-alt bx-xs'
                    style="margin-bottom: 3px;"></i>&nbsp;Kembali</button>
            <div style="margin-top: 3px;"><i class='bx bx-history bx-spin text-primary mb-1' data-bs-toggle="tooltip"
                    data-popup="tooltip-custom" data-bs-placement="auto"
                    title="Tanggal Sumbit Quiz"></i>&nbsp;{{ $dataresult->created_at->locale('id')->isoFormat('D MMMM YYYY | H:mm') }}
            </div>
        </div>
        ...
        @foreach ($scores as $index => $score)
            <div class="d-flex">
                ...
                @else
                    <p style="font-size: 1.3rem; margin-top:-5px; margin-left: 10px;">{{ $score->question->question }}
                    </p>
                @endif
            </div>
