        ...
        <div class="card h-100">
            <div class="card-header d-flex align-items-center justify-content-between" style="margin-bottom: -0.7rem;">
                <div class="justify-content-start">
                    <select data-url="{{ $dataquiz->slug }}" id="score-filter" class="form-select"
                        style="border: 1px solid #d9dee3;">
                        <option disabled selected>Filter Nilai</option>
                        <option value="tertinggi" @if (request('filter') === 'tertinggi') selected @endif>Nilai Tertinggi
                        </option>
                                ...
                                @foreach ($reports as $index => $data)
                                    <tr>
                                        ...
                                        <td><i data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                ...
                                                class="badge bg-label-primary">{{ $data->answer_user->where('answer_id', !null)->count() }}
                                                Soal dikerjakan</span></td>
                                        <td>{!! $data->quiz->load('question')->question->count() - $data->answer_user->where('answer_id', !null)->count() > 0
                                            ? '<i data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="right" title="Danger Icon" class="bx bx-error bx-flashing text-danger" style="font-size: 19px;"></i>&nbsp;<span class="badge bg-label-danger">' .
                                                $data->quiz->load('question')->question->count() -
                                                $data->answer_user->where('answer_id', !null)->count() .
                                        ...
                                        <td class="text-center"><span
                                                class="badge badge-center bg-dark rounded-pill">{{ $data->score }}</span>
                                        </td>
