...
<div class="flash-message-pengguna"
    data-add-p-user="@if (session()->has('adduserSuccess')) {{ session('adduserSuccess') }} @endif"
    data-edit-p-user="@if (session()->has('updateUserSuccess')) {{ session('updateUserSuccess') }} @endif"
    data-delete-p-user="@if (session()->has('deleteUserSuccess')) {{ session('deleteUserSuccess') }} @endif"
    data-error-delete-p-user="@if (session()->has('deleteUserError')) {{ session('deleteUserError') }} @endif"></div>
<div class="row">
            ...
            <div class="card-header d-flex align-items-center justify-content-between" style="margin-bottom: -0.7rem;">
                <div class="justify-content-start">
                    <button type="button" class="btn btn-xs btn-dark fw-bold p-2" data-bs-toggle="modal"
                        data-bs-target="#formModalAdminAddPengguna" style="border-radius: 0.375rem;">
                    ...
                    <form action="/admin/pengguna/search">
                        <div class="input-group">
                            <input type="search" class="form-control" name="q" id="search"
                                ...
                                placeholder="Cari Data Pengguna..." autocomplete="off" />
                        </div>
                    </form>
                    <!-- /Search -->
                </div>
                            ...
                            <tbody class="table-border-bottom-0">
                                @foreach ($users as $index => $user)
                                    <tr>
                                        ...
                                        <td>
                                            <img src="@if (Storage::disk('public')->exists($user->image)) {{ asset('storage/' . $user->image) }} @else {{ asset('assets/img/' . $user->image) }} @endif"
                                                alt="Foto Profil - {{ $user->name }}"
                                                class="rounded avatar avatar-sm fotoProfile"
                                                data-url-img="@if (Storage::disk('public')->exists($user->image)) {{ asset('storage/' . $user->image) }} @else {{ asset('assets/img/' . $user->image) }} @endif"
                                        ...
                                        <td>
                                            @if ($user->tanggal_lahir)
                                                {{ \Carbon\Carbon::parse($user->tanggal_lahir)->locale('id')->isoFormat('D MMMM YYYY') }}
                                            @else
                                                Belum diinput pengguna!
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <button type="button"
                                                class="btn btn-icon btn-primary btn-sm buttonEditPengguna"
                                                data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                data-bs-placement="auto" title="Edit Pengguna"
                                                data-id="{{ encrypt($user->id) }}">
                                                <span class="tf-icons bx bx-edit" style="font-size: 15px;"></span>
                                            </button>
                                            <button type="button"
                                                class="btn btn-icon btn-danger btn-sm buttonDeletePengguna"
                                                data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                data-bs-placement="auto" title="Hapus Pengguna"
                                ...
                                @if ($users->isEmpty())
                                    <tr>
                                        <td colspan="100" class="text-center">Data pengguna tidak ditemukan dengan
                                            keyword pencarian: <b>"{{ request('q') }}"</b></td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
        ...
        <form action="/admin/pengguna" method="post" class="modalAdminAddPengguna" enctype="multipart/form-data">
            ...
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between">
                    <h5 class="modal-title text-primary fw-bold">Tambah Pengguna Baru&nbsp;<i class='bx bxs-user fs-5'
                            style="margin-bottom: 1px;"></i></h5>
                    <button type="button" class="btn p-0 dropdown-toggle hide-arrow cancelModalAddUser"
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col mb-3">
                            ...
                            <input type="text" id="nama_lengkap_user" name="name" value="{{ old('name') }}"
                    ...
                    <div class="row">
                        <div class="col mb-3">
                            ...
                            <input type="text" id="username_user" name="username" value="{{ old('username') }}"
                                class="form-control @error('username') is-invalid @enderror"
                                placeholder="Masukkan username pengguna" autocomplete="off" required>
                            @error('username')
                                <div class="invalid-feedback" style="margin-bottom: -3px;">
                                    {{ $message }}
                                </div>
                            @enderror
                            <div class="form-text @error('username') d-none @enderror">Username minimal 5 karakter
                                (Hanya boleh huruf & angka).</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col mb-3">
                            <label for="email_user" class="form-label required-label">Email</label>
                            <input type="text" id="email_user" name="email" value="{{ old('email') }}"
                                class="form-control @error('email') is-invalid @enderror"
                                placeholder="Masukkan email pengguna" autocomplete="off" required>
                            @error('email')
                                <div class="invalid-feedback" style="margin-bottom: -3px;">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row">
                        <div class="col mb-3">
                            ...
                            <input type="file" id="profil_user" name="image"
                            ...
                            @if ($errors->has('image'))
                                ...
                                @endforeach
                            @endif
                            <div class="form-text @error('image') d-none @enderror">Ukuran maks 500 KB dengan rasio
                                1:1. Format: JPG, PNG, JPEG.</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col">
                            <label for="password_user" class="form-label required-label">Kata Sandi</label>
                            ...
                            @if ($errors->has('password'))
                                ...
                                @endforeach
                            @endif
                            <div class="form-text @error('password') d-none @enderror"><i class='bx bx-error'
                                    style="font-size: 100%;"></i>&nbsp;Kata sandi minimal 8 karakter termasuk huruf
                                kapital & kecil (A-Z), (a-z), angka (1-9), dan karakter unik (@,#,%,dll)</div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-danger cancelModalAddUser"
...
@endif
<!-- Modal Edit Pengguna-->
<div class="modal fade" id="formModalAdminEditPengguna" data-bs-backdrop="static" tabindex="-1"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="/admin/pengguna/edit" method="post" class="modalAdminEditPengguna"
            enctype="multipart/form-data">
            <input type="hidden" name="codeUser" value="{{ old('codeUser') }}" id="codeUser">
            @csrf
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between">
                    <h5 class="modal-title text-primary fw-bold">Edit Pengguna&nbsp;<i class='bx bxs-user fs-5'
                            style="margin-bottom: 1px;"></i></h5>
                    <button type="button" class="btn p-0 dropdown-toggle hide-arrow cancelModalEditUser"
                        data-bs-dismiss="modal"><i class="bx bx-x-circle text-danger fs-4" data-bs-toggle="tooltip"
                            data-popup="tooltip-custom" data-bs-placement="auto" title="Tutup"></i></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col mb-3">
                            ...
                            <input type="text" id="edit_nama_lengkap_user" name="name"
                                value="{{ old('name') }}"
                                class="form-control @if (session('validationErrors')) @foreach (session('validationErrors')['name'] ?? [] as $errorMessage) is-invalid @endforeach @endif"
                                placeholder="Masukkan nama pengguna" autocomplete="off" required>
                            @if (session('validationErrors'))
                                @foreach (session('validationErrors')['name'] ?? [] as $errorMessage)
                                    <div class="invalid-feedback" style="margin-bottom: -3px;">
                                        {{ $errorMessage }}
                                    </div>
                                @endforeach
                    ...
                    <div class="row">
                        <div class="col mb-3">
                            ...
                            <input type="text" id="edit_username_user" name="username"
                                value="{{ old('username') }}"
                                class="form-control @if (session('validationErrors')) @foreach (session('validationErrors')['username'] ?? [] as $errorMessage) is-invalid @endforeach @endif"
                                placeholder="Masukkan username pengguna" autocomplete="off" required>
                            @if (session('validationErrors'))
                                @foreach (session('validationErrors')['username'] ?? [] as $errorMessage)
                                    <div class="invalid-feedback" style="margin-bottom: -3px;">
                                        {{ $errorMessage }}
                                    </div>
                                @endforeach
                            @endif
                            <div
                                class="form-text @if (session('validationErrors')) @foreach (session('validationErrors')['username'] ?? [] as $errorMessage) d-none @endforeach @endif">
                                Username minimal 5 karakter (Hanya boleh huruf & angka).</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col mb-3">
                            <label for="edit_email_user" class="form-label required-label">Email</label>
                            <input type="text" id="edit_email_user" name="email" value="{{ old('email') }}"
                                class="form-control @if (session('validationErrors')) @foreach (session('validationErrors')['email'] ?? [] as $errorMessage) is-invalid @endforeach @endif"
                                placeholder="Masukkan email pengguna" autocomplete="off" required>
                            @if (session('validationErrors'))
                                @foreach (session('validationErrors')['email'] ?? [] as $errorMessage)
                                    <div class="invalid-feedback" style="margin-bottom: -3px;">
                                        {{ $errorMessage }}
                                    </div>
                                @endforeach
                    ...
                    <div class="row">
                        <div class="col mb-3">
                            ...
                            <input type="file" id="edit_profil_user" name="image"
                                class="form-control @if (session('validationErrors')) @foreach (session('validationErrors')['image'] ?? [] as $errorMessage) is-invalid @endforeach @endif">
                            @if (session('validationErrors'))
                                @foreach (session('validationErrors')['image'] ?? [] as $errorMessage)
                                    <div class="invalid-feedback" style="margin-bottom: -3px;">
                                        {{ $errorMessage }}
                                    </div>
                                @endforeach
                            @endif
                            <div
                                class="form-text @if (session('validationErrors')) @foreach (session('validationErrors')['image'] ?? [] as $errorMessage) d-none @endforeach @endif">
                                Ukuran maks 500 KB dengan rasio 1:1. Format: JPG, PNG, JPEG.</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col">
                            <label for="edit_password_user" class="form-label">Kata Sandi</label>
                            <input type="password" id="edit_password_user" name="password"
                                class="form-control @if (session('validationErrors')) @foreach (session('validationErrors')['password'] ?? [] as $errorMessage) is-invalid @endforeach @endif"
                                placeholder="&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;&#xb7;"
                                autocomplete="off">
                            @if (session('validationErrors'))
                                @foreach (session('validationErrors')['password'] ?? [] as $errorMessage)
                                    <div class="invalid-feedback" style="margin-bottom: -3px;">
                                        {{ $errorMessage }}
                                    </div>
                                @endforeach
                            @endif
                            <div
                                class="form-text @if (session('validationErrors')) @foreach (session('validationErrors')['password'] ?? [] as $errorMessage) d-none @endforeach @endif">
                                <i class='bx bx-error' style="font-size: 100%;"></i>&nbsp;Kata sandi minimal 8
                                karakter
                                termasuk huruf kapital & kecil (A-Z), (a-z), angka (1-9), dan karakter unik (@,#,%,dll)
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-danger cancelModalEditUser"
                        data-bs-dismiss="modal"><i class='bx bx-share fs-6'
                            style="margin-bottom: 3px;"></i>&nbsp;Batal</button>
