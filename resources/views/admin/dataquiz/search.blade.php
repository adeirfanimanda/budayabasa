...
@endsection
<div class="flash-message" data-add-quiz="@if (session()->has('quizSuccess')) {{ session('quizSuccess') }} @endif"
    data-edit-quiz="@if (session()->has('updateQuizSuccess')) {{ session('updateQuizSuccess') }} @endif"
    data-delete-quiz="@if (session()->has('deleteQuizSuccess')) {{ session('deleteQuizSuccess') }} @endif"
    data-error-delete-quiz="@if (session()->has('deleteQuizError')) {{ session('deleteQuizError') }} @endif"></div>
<div class="row">
    <div class="col-md-12 col-lg-12 order-2 mb-4">
        <div class="card h-100">
            <div class="card-header d-flex align-items-center justify-content-between" style="margin-bottom: -0.7rem;">
                <div class="justify-content-start">
                    <button type="button" class="btn btn-xs btn-dark fw-bold p-2 buttonAddQuiz" data-bs-toggle="modal"
                        data-bs-target="#formModalAdminQuiz" style="border-radius: 0.375rem;">
                        <i class='bx bx-joystick fs-6'></i>&nbsp;TAMBAH LATIHAN
                    </button>
                </div>
                    ...
                    <form action="/admin/data-quiz/search">
                        <div class="input-group">
                            <input type="search" class="form-control" name="q" id="search"
                                ...
                                placeholder="Cari Data Latihan..." autocomplete="off" />
                        </div>
                    </form>
                </div>
            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($quiz->status == 'Aktif')
                                                <span
                                                class="badge bg-label-success fw-bold">{{ $quiz->status }}</span>@else<span
                                                    class="badge bg-label-danger fw-bold">{{ $quiz->status }}</span>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-icon btn-primary btn-sm"
                                                onclick="window.location.href='/admin/data-quiz/q&a/{{ $quiz->slug }}'"
                                                data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                data-bs-placement="auto" title="Pertanyaan & Jawaban">
                                                <span class="tf-icons bx bx-show" style="font-size: 15px;"></span>
                                            </button>
                                            <button type="button"
                                                class="btn btn-icon btn-warning btn-sm buttonEditQuiz"
                                                data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                data-bs-placement="auto" title="Edit Latihan"
                                            ...
                                            <button type="button"
                                                class="btn btn-icon btn-danger btn-sm buttonDeleteQuiz"
                                                data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                data-bs-placement="auto" title="Hapus Latihan"
                                                data-code-quiz="{{ $quiz->slug }}"
                                                data-title-quiz="{{ $quiz->title }}">
                                ...
                                @if ($allQuiz->isEmpty())
                                    <tr>
                                        <td colspan="100" class="text-center">
                                            Data latihan tidak ditemukan dengan keyword pencarian:
                                            <b>"{{ request('q') }}"</b>
                                        </td>
                                    </tr>
                                @endif
        ...
        <form action="/admin/data-quiz" method="post" class="modalAdminQuiz">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col mb-3">
                            <label for="title" class="form-label required-label">Judul Latihan</label>
                            <input type="text" id="title" name="title" value="{{ old('title') }}"
                                class="form-control @error('title') is-invalid @enderror"
                                placeholder="Masukkan judul latihan" autocomplete="off" required>
                            @error('title')
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-danger cancelModalAddQuiz"
...
<div class="modal fade" id="formEditModalAdminQuiz" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="/admin/data-quiz/update" method="post" class="modalAdminQuiz">
            ...
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between">
                    <h5 class="modal-title text-primary fw-bold">Edit Latihan&nbsp;<i class='bx bx-joystick fs-5'
                            style="margin-bottom: 1px;"></i></h5>
                    <button type="button" class="btn p-0 dropdown-toggle hide-arrow cancelModalEditQuiz"
                        data-bs-dismiss="modal"><i class="bx bx-x-circle text-danger fs-4" data-bs-toggle="tooltip"
                            data-popup="tooltip-custom" data-bs-placement="auto" title="Tutup"></i></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col mb-3">
                            <label for="titleEdit" class="form-label required-label">Judul Latihan</label>
                            <input type="text" id="titleEdit" name="titleQuiz" value="{{ old('titleQuiz') }}"
                                class="form-control @error('titleQuiz') is-invalid @enderror" autocomplete="off"
                                placeholder="Masukkan judul latihan" required>
                            @error('titleQuiz')
                                <div class="invalid-feedback" style="margin-bottom: -3px;">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            <textarea class="form-control @error('descriptionQuiz') is-invalid @enderror" id="deskripsiEdit"
                                ...
                                autocomplete="off" required>{{ old('descriptionQuiz') }}</textarea>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-danger cancelModalEditQuiz"
                        data-bs-dismiss="modal"><i class='bx bx-share fs-6'
                            style="margin-bottom: 3px;"></i>&nbsp;Batal</button>
