                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap mb-2 gap-1">
                    <div class="me-1 mb-2">
                        ...
                        <small class="text-muted">Dipost oleh <strong
                                class="text-primary">{{ $thread->user->name }}</strong>
                            {{ $thread->created_at->locale('id')->diffForHumans() }}</small>
                    </div>
                    <div class="d-flex">
                        <div class="dropdown">
                            <div class="d-flex align-items-center d-lg-block d-none cursor-pointer me-1"
                                data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <i class='bx bxs-heart text-danger fs-5 bx-sm mx-1'
                                    style="margin-bottom: 2px;"></i>{{ $thread->like->count() }} Suka
                            </div>
                            <div class="dropdown-menu dropdown-menu">
                                @foreach ($thread->like->load('thread', 'user') as $userLikes)
                                    <a class="dropdown-item">{{ $userLikes->user->name }} <strong class="text-danger"
                                            style="font-size: 10px;">Liked</strong></a>
                                @endforeach
                            </div>
                        </div>
                        <div class="d-flex align-items-center d-lg-none cursor-pointer me-1" data-bs-toggle="dropdown"
                            aria-haspopup="true" aria-expanded="false">
                            <i class='bx bxs-heart text-danger fs-5 bx-sm mx-1'
                                style="margin-bottom: 2px;"></i>{{ $thread->like->count() }}
                        </div>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu">
                                <a class="dropdown-item" target="_blank"
                                    href="https://api.whatsapp.com/send?phone=&text={{ $thread->title . '%0A%0A' . url()->current() }}"><i
                                        class='bx bxl-whatsapp fs-5 bx-sm mb-1 me-2'
                                        style="color:#25D366"></i>WhatsApp</a>
                                <a class="dropdown-item" target="_blank"
                                    href="https://www.facebook.com/sharer.php?u={{ url()->current() }}"><i
                                        class='bx bxl-facebook-square fs-5 bx-sm mb-1 me-2'
                        ...
                        <div id="topic" class="accordion-collapse collapse show">
                            <div class="accordion-body py-3 border-top">
                                ...
                                @foreach ($threads as $newsthread)
                                    @if ($newsthread->id != $thread->id && $i <= 5)
                                        <div class=" d-flex align-items-center mb-3"
                                            onclick="window.location.href='/view/discuss/thread/{{ $newsthread->id }}'">
                                            <label class="form-check-label cursor-pointer">
                                                <span class="mb-0 h6"> {{ $i++ }}. {{ $newsthread->title }}
                                                </span>
                                                <small class="text-muted d-block">Dipost oleh <strong
                                                        class="text-primary">{{ $newsthread->user->name }}</strong>
                                                    {{ $newsthread->created_at->locale('id')->diffForHumans() }}</small>
                                            </label>
                                        </div>
                                    @endif
                                @endforeach
