...
<div class="flash-message" data-add-topic="@if (session()->has('addTopicSuccess')) {{ session('addTopicSuccess') }} @endif"
    data-delete-topic="@if (session()->has('deleteTopicSuccess')) {{ session('deleteTopicSuccess') }} @endif"></div>
<div class="card mb-4">
    <div class="card-header mb-1">
        <div class="d-flex justify-content-between">
            <div>
                <button type="button" class="btn btn-xs btn-dark fw-bold p-2" data-bs-toggle="modal"
                    data-bs-target="#formModalAddTopic" style="border-radius: 0.375rem;">
    ...
    <div class="card-body">
                                ...
                                <div class="card-actions">
                                    ...
                                    <div class="d-flex">
                                        <div class="like-container">
                                            <a data-thread="{{ $thread->id }}"
                                                class="text-muted likebutton cursor-pointer me-3">
                                                @if ($userLiked)
        ...
        @if ($threads->isEmpty())
            <span class="d-flex justify-content-center mb-2"><i class='bx bx-info-circle fs-5'
                    style="margin-top: 3px;"></i>&nbsp;Data tidak ditemukan dengan keyword pencarian:
                <b>"{{ request('q') }}"</b></span>
        @endif
...
<div id="errorModalAddTopic" data-error-title="@error('title') {{ $message }} @enderror"
    data-error-content="@error('content') {{ $message }} @enderror"></div>
        ...
        <form action="/view/discuss" method="post" class="modalAddTopic">
            ...
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between">
                    <h5 class="modal-title text-primary fw-bold">Tambah Thread Baru&nbsp;<i
                            class='bx bx-message-dots fs-5' style="margin-bottom: 1px;"></i></h5>
                    <button type="button" class="btn p-0 dropdown-toggle hide-arrow cancelModalAddTopic"
                    ...
                    <div class="row">
                        <div class="col mb-3">
                            ...
                            <input type="text" id="title" name="title" value="{{ old('title') }}"
                                ...
                                placeholder="Masukkan judul thread" autocomplete="off" required>
                            @error('title')
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content"
                                autocomplete="off" placeholder="Masukkan deskripsi thread" rows="4" required>{{ old('content') }}</textarea>
                            @error('content')
                                <div class="invalid-feedback" style="margin-bottom: -3px;">
                                    {{ $message }}
                                </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-danger cancelModalAddTopic"
