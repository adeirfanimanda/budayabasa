...
<div class="card mb-4">
    <div class="card-header">
        <div class="card-title mb-0 me-1">
            <h5 class="mb-1">Latihan&nbsp;<i class="bx bx-joystick fs-5"></i></h5>
            <p class="text-muted mb-0">Berikut @if ($quizzes->count() > 1)
                    kumpulan
                @endif latihan yang bisa anda kerjakan.</p>
        </div>
    </div>
    <div class="card-body">
                    ...
                    <div class="card p-2 h-100 shadow-none border">
                        <div class="card-body p-3 pt-2">
                            <div class="d-flex justify-content-between mt-2">
                                ...
                                <div>
                                    <button type="button" data-title-quiz="{{ $quiz->title }}"
                                        data-description-quiz="{{ $quiz->description }}"
                                        data-total-soal-quiz="{{ $quiz->question->count() }}"
                                        class="btn p-0 dropdown-toggle hide-arrow buttonDetails" data-bs-toggle="modal"
                                        data-bs-target="#quizDetails"><i class="bx bx-info-circle mb-2 text-primary"
                                            data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                            data-bs-placement="auto"
                                            title="Klik untuk melihat detail latihan"></i></button>
                                </div>
                            </div>
                            <h6 class="card-subtitle text-muted" style="font-size: 12px; margin-bottom:10px;">Dibuat
                                {{ $quiz->created_at->locale('id')->diffForHumans() }}</h6>
                            ...
                            <button type="button" class="w-100 btn btn-label-primary"
                                ...
                                <i class='bx bx-joystick fs-5' style="margin-bottom: 3px;"></i>&nbsp;Mulai
                                Latihan</button>
                        </div>
                    </div>
                </div>
        ...
        @if ($quizzes->isEmpty())
            <span class="d-flex justify-content-center mb-2"><i class='bx bx-info-circle fs-5'
                    style="margin-top: 3px;"></i>&nbsp;Latihan tidak ditemukan!</span>
        @endif
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header d-flex justify-content-between">
                <h5 class="modal-title text-primary fw-bold">Detail Latihan&nbsp;<i class='bx bx-joystick fs-5'
                        style="margin-bottom: 2px;"></i></h5>
