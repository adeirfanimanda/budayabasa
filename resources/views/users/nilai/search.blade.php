    ...
    <div class="col-md-12 col-lg-12 order-2 mb-4">
        <div class="card h-100">
            <div class="card-header d-flex align-items-center justify-content-between" style="margin-bottom: -0.7rem;">
                <div class="justify-content-start">
                    <select id="score-filter" class="form-select" style="border: 1px solid #d9dee3;">
                        ...
                        <option value="terendah" @if (request('filter') === 'terendah') selected @endif>Nilai Terendah
                        </option>
                    </select>
                </div>
                    ...
                    <form action="/nilai/search">
                        <div class="input-group">
                            <input type="search" class="form-control" name="q" id="search"
                                ...
                                placeholder="Cari Histori Latihan..." autocomplete="off" />
                        </div>
                    </form>
                </div>
            </div>
            <div class="card-body">
                        ...
                        <table class="table table-striped">
                            <thead class="table-dark">
                                <tr>
                                    ...
                                    <th class="text-white">Tanggal Submit Latihan</th>
                                    ...
                                    <tr>
                                        ...
                                        @else
                                            <td style="font-size: 18px;">
                                                {{ Str::limit($history->quiz->title, 21, '...') }}</td>
                                        @endif
                                        <td>{{ Str::limit($history->quiz->description, 50, '...') }}</td>
                                        <td class="text-center"><span
                                                class="badge badge-center bg-dark rounded-pill">{{ $history->quiz->load('question')->question->count() }}</span>
                                        </td>
                                        <td><i data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                data-bs-placement="right" title="Task Icon"
                                                class="bx bx-task bx-tada text-primary"
                                                style="font-size: 19px;"></i>&nbsp;<span
                                                class="badge bg-label-primary">{{ $history->answer_user->where('answer_id', !null)->count() }}
                                                Soal dikerjakan</span></td>
                                        <td>{!! $history->quiz->load('question')->question->count() -
                                            $history->answer_user->where('answer_id', !null)->count() >
                                        0
                                            ? '<i data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="right" title="Danger Icon" class="bx bx-error bx-flashing text-danger" style="font-size: 19px;"></i>&nbsp;<span class="badge bg-label-danger">' .
                                                $history->quiz->load('question')->question->count() -
                                                $history->answer_user->where('answer_id', !null)->count() .
                                                ' Soal tidak dikerjakan</span>'
                                            : '<i data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="right" title="Check Icon" class="bx bx-check-double bx-tada text-success" style="font-size: 20px;"></i>&nbsp;<span class="badge bg-label-dark">Dikerjakan Semua</span>' !!}</td>
                                        <td>{{ $history->created_at->locale('id')->isoFormat('D MMMM YYYY | H:mm') }}
                                        </td>
                                        <td class="text-center"><span
                                                class="badge badge-center bg-dark rounded-pill">{{ $history->score }}</span>
                                        </td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-icon btn-primary btn-sm"
                                                ...
                                                <span data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                    data-bs-placement="auto" title="Details" class="tf-icons bx bx-show"
                                            ...
                                            <button type="button"
                                                ...
                                                <span data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                                    data-bs-placement="auto" title="Hapus Histori Latihan"
                                                    class="tf-icons bx bx-trash" style="font-size: 14px;"></span>
                                            </button>
                                        </td>
                                    </tr>
                                @endforeach
                ...
                @if (!$histories->isEmpty())
                    <div class="mt-3 pagination-mobile">{{ $histories->withQueryString()->onEachSide(1)->links() }}
                    </div>
                @endif
            </div>
        </div>
