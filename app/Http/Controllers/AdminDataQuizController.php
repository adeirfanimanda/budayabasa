...
class AdminDataQuizController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect('/admin/data-quiz')->with('quizSuccess', 'Latihan berhasil ditambahkan!');
    }
    ...
    public function show(Quiz $quiz)
    {
        return view('admin.dataquiz.show', [
            'app' => Application::all(),
            'title' => 'Pertanyaan & Jawaban Latihan',
            'code' =>  $quiz->slug,
    ...
    public function addquestion(Quiz $quiz)
    {
        ...
        if ($questions) {
            ...
            if ($answerUserCount > 0) {
                return redirect('/admin/data-quiz/q&a/' . $quiz->slug)->with('updateQuestionError', 'Data latihan digunakan users!');
                exit;
            }
        }
    ...
    public function update(Request $request)
    {
        ...
        Quiz::where('slug', $request->codeQuiz)
            ->update($validated);
        return back()->with('updateQuizSuccess', 'Latihan berhasil diupdate!');
    }
    ...
    public function destroy(Quiz $quiz)
    {
        ...
        if ($deleteQuizError) {
            return back()->with('deleteQuizError', 'Data latihan digunakan users!');
            exit;
        }
        Question::where('quiz_id', $quiz->id)->delete();
        Quiz::destroy($quiz->id);
        return back()->with('deleteQuizSuccess', 'Latihan berhasil dihapus!');
    }
